"""Local OCR extraction of structured fields from a CIN image.
...
This is inherently less accurate than a vision model: it depends on layout
assumptions and OCR line order surviving intact. Fields that can't be found
are returned as "" rather than failing the whole request. The request only
fails if neither a CIN number nor a name could be found at all.
"""
import logging
import re
...
def extract_cin(image_bytes: bytes) -> dict:
    """Extract structured CIN fields from a card image using local OCR.
