...
interface FactureLayoutEditorProps {
    initialBackground?: string
    initialLayout?: any
    onSave: (background: string, layout: any) => void
}

// Element ids drawn on the facture canvas / print page.
const ELEMENT_IDS = ["patient_name", "date", "acts_table", "totals", "footer"] as const

export default function FactureLayoutEditor({ initialBackground, initialLayout, onSave }: FactureLayoutEditorProps) {
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            const response = await apiClient.uploadFactureBackground(formData)
            ...
            <div className="lg:col-span-4 space-y-6">
                ...
                <div className="bg-blue-50 border border-blue-100 rounded-xl p-4 space-y-2">
                    ...
                    <ul className="text-xs text-blue-700 space-y-1 ml-4 list-disc">
                        <li>Le tableau des actes affichera les actes du rendez-vous</li>
...
// Draws a small sample invoice table preview on the editor canvas.
function drawSampleTable(ctx: CanvasRenderingContext2D, px: number, py: number, fontSize: number, canvasWidth: number, isSel: boolean) {
    const rowH = fontSize + 10
    const tableW = Math.min(canvasWidth * 0.55, canvasWidth - px - 20)
    const col2 = tableW * 0.68
    const rows = [
        ["Désignation", "Prix (DH)"],
        ["Consultation", "250"],
        ["Échographie", "400"],
    ]
    ctx.strokeStyle = isSel ? "#2563eb" : "#666"
    ctx.lineWidth = 1
    ctx.font = `${fontSize}px Arial`
    rows.forEach((row, i) => {
        const ry = py + i * rowH
        ctx.strokeRect(px, ry, col2, rowH)
        ctx.strokeRect(px + col2, ry, tableW - col2, rowH)
        ctx.fillStyle = i === 0 ? "#111" : (isSel ? "#2563eb" : "#333")
        ctx.font = `${i === 0 ? 'bold ' : ''}${fontSize}px Arial`
        ctx.fillText(row[0], px + 4, ry + fontSize + 2)
        ctx.fillText(row[1], px + col2 + 4, ry + fontSize + 2)
    })
}
